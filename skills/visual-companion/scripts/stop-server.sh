...
# under /tmp (ephemeral). Persistent directories (.mockup/) are
...
command_has_server_id() {
  local pid="$1"
  local expected="$2"
  local expected_arg="--visual-companion-server-id=$expected"
  if [[ -r "/proc/$pid/cmdline" ]]; then
    local arg
...
if [[ -f "$PID_FILE" ]]; then
  ...
  if ! is_visual_companion_server "$pid"; then
